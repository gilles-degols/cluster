...
/**
  ...
  * The OrderId comes from the WorkerTypeOrder, to distinguish workers for the same WorkerType, but belonging to different
  * orders (aka, "sub-groups" in this case)
  */
class Worker(val workerId: String, val orderId: String, val actorRef: Option[ActorRef]) extends ClusterElement{
...
object Worker {
  def fromWorkerIdAndActorRef(workerId: String, orderId: String, actorRef: Option[ActorRef]): Worker = {
    val worker = new Worker(workerId, orderId, actorRef)
    worker
  }
